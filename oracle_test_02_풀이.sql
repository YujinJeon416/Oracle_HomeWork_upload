...
select emp_name, job_code, to_char((salary+(nvl(bonus,0)*salary))*12,'L999,999,999,999') as 연봉
from employee;
...
--  * 주말도 포함 , 소수점 아래는 버림
--  * 퇴사자는 퇴사일-입사일로 계산
select emp_name, hire_date, trunc(nvl(quit_date,sysdate)-hire_date) as 근무일수
from employee;
...
--   나이는 주민번호에서 추출해서 날짜데이터로 변환한 다음, 계산함
    SELECT EMP_NAME, DEPT_CODE, 
        ...
        trunc(months_between(sysdate, (decode(substr(emp_no,8,1),'1',1900,'2',1900,2000)+substr(emp_no,1,2)) || substr(emp_no,3,4))/12) 나이
    FROM EMPLOYEE;
...
where quit_yn = 'N';
select hire_date,
       decode(extract(year from hire_date),1998,1),
       decode(extract(year from hire_date),1999,1),
       decode(extract(year from hire_date),2000,1),
       decode(extract(year from hire_date),2001,1),
       decode(extract(year from hire_date),2002,1),
       decode(extract(year from hire_date),2003,1),
       decode(extract(year from hire_date),2004,1)
from employee;
